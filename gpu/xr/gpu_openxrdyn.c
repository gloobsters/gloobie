...
void GPU_OpenXR_UnloadLibrary(void)
{
...
bool GPU_OpenXR_LoadLibrary(void)
{
    bool result = true;
    ...
    if (openxr_load_refcount++ == 0)
    {
        const char *paths_hint = SDL_GetHint(GPU_HINT_OPENXR_SONAMES);
...
PFN_xrGetInstanceProcAddr GPU_OpenXR_GetXrGetInstanceProcAddr(void)
{
